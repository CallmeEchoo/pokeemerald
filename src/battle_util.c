...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
            ...
            if ((gBattleMons[battler].status2 & STATUS2_WRAPPED) && IsBattlerAlive(battler))
            {
                if (--gDisableStructs[battler].wrapTurns != 0)  // damaged by wrap
                {
                    ...
                    if (GetBattlerHoldEffect(gBattleStruct->wrappedBy[battler], TRUE) == HOLD_EFFECT_BINDING_BAND
                     && GetBattlerAbility(gBattleStruct->wrappedBy[battler]) == ABILITY_TIGHT_GRIP)
                        gBattleMoveDamage = GetNonDynamaxMaxHP(battler) / (B_BINDING_DAMAGE >= GEN_6 ? 4 : 6);
                    else if (GetBattlerHoldEffect(gBattleStruct->wrappedBy[battler], TRUE) == HOLD_EFFECT_BINDING_BAND
                     || GetBattlerAbility(gBattleStruct->wrappedBy[battler]) == ABILITY_TIGHT_GRIP)
        ...
        case ENDTURN_SEA_OF_FIRE_DAMAGE:
            ...
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_SOUND_CHARGED:  // charge
            if (gDisableStructs[battler].soundCharged && --gDisableStructs[battler].soundCharged == 0)
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_BRUTE:
            ...
            break;
        case ABILITY_SOOTHING_SCENT:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                SET_STATCHANGER(STAT_SPATK, 1, TRUE);
                BattleScriptPushCursorAndCallback(BattleScript_SoothingScentActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_CALTROPS:
             ...
             && (gSideTimers[GetBattlerSide(gBattlerAttacker)].spikesAmount < 3))
            {
                SWAP(gBattlerAttacker, gBattlerTarget, i);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_CaltropsActivates;
                effect++;
            }
            break;
        case ABILITY_RESONANT_BODY:
            ...
            {
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_ResonantBodyActivates;
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_BITE_DOWN:
            ...
            break;
        case ABILITY_FROSTY_SPIRIT:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && IsBattlerAlive(gBattlerTarget)
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && CanBeFrozen(gBattlerTarget)
             && GetMoveType(move) == TYPE_GHOST
             && TARGET_TURN_DAMAGED
             && RandomPercentage(RNG_POISON_TOUCH, 30))
            {
    ...
    case ABILITYEFFECT_ON_WEATHER: // For ability effects that activate when the battle weather changes.
        ...
        case ABILITY_DESERT_DWELLER:
             ...
             && GetActiveGimmick(battler) != GIMMICK_TERA)
            {
                ...
                BattleScriptPushCursorAndCallback(BattleScript_DesertDwellerActivates);
                effect++;
            }
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_ERUPTION:
        if (GetBattlerAbility(battlerAtk) != ABILITY_COAL_POWERED || gBattleMons[battlerAtk].item != ITEM_CHARCOAL)
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    if (gStatuses3[battlerAtk] & STATUS3_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (gStatuses4[battlerAtk] & STATUS4_SOUND_CHARGED && gMovesInfo[move].soundMove)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
...
static inline u32 CalcDefenseStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    case ABILITY_BRUTE:
        if (usesDefStat)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            if (updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_BRUTE);
        }
        break;
    }
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if (moveType == TYPE_POISON && abilityAtk == ABILITY_POLLUTION
        && (defType == TYPE_WATER || defType == TYPE_GROUND))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, abilityAtk);
...
uq4_12_t GetTypeEffectiveness(struct Pokemon *mon, u8 moveType)
{
    ...
    if (moveType != TYPE_MYSTERY)
    {
        ...
        if ((modifier <= UQ_4_12(1.0)  &&  abilityDef == ABILITY_WONDER_GUARD)
         ...
         || (moveType == TYPE_STEEL    && (abilityDef == ABILITY_IRONCLAD)))
        {
            modifier = UQ_4_12(0.0);
        }
    }
