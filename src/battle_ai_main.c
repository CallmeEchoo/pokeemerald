    ...
    // other
        ...
        case EFFECT_RECOIL_IF_MISS:
            if (aiData->abilities[battlerAtk] != ABILITY_MAGIC_GUARD
             && aiData->abilities[battlerAtk] != ABILITY_COILED_LEGS
             && aiData->moveAccuracy[battlerAtk][battlerDef][AI_THINKING_STRUCT->movesetIndex] < 75
        ...
        case EFFECT_LASER_FOCUS:
            if (gStatuses3[battlerAtk] & STATUS3_LASER_FOCUS)
               ...
               || aiData->abilities[battlerDef] == ABILITY_SIXTH_SENSE)
                ADJUST_SCORE(-8);
            break;
        case EFFECT_SKETCH:
            if (gLastMoves[battlerDef] == MOVE_NONE)
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_CONTRARY:
                ...
                break;
            case ABILITY_IRONCLAD:
                if (moveType == TYPE_STEEL && GetNoOfHitsToKOBattler(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex) >= 4)
                {
                    RETURN_SCORE_PLUS(WEAK_EFFECT);   // only mon with this ability is weak to water so only make it okay if we do very little damage
                }
                RETURN_SCORE_MINUS(10);
